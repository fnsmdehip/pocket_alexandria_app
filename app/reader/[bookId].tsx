    ...
    <View style={[styles.screen, { backgroundColor: themeColors.background }]}>
      ...
      >
        ...
        >
          <Text
            style={[
              styles.pageText,
              { color: themeColors.text, fontSize, lineHeight: fontSize * lineHeight },
            ]}
          >
            {pages[currentPage] || ''}
          </Text>
      ...
      <View style={[styles.pageIndicator, { backgroundColor: themeColors.surface }]}>
        ...
        <View style={styles.progressTrack}>
          <View style={[styles.progressFill, { width: `${progress}%`, backgroundColor: themeColors.accent }]} />
        </View>
      </View>
        ...
        <View style={styles.controlsOverlay}>
          <View style={[styles.topBar, { backgroundColor: colors.overlay }]}>
            ...
            <View style={styles.topTitle}>
              <Text style={styles.topTitleText} numberOfLines={1}>{book.title}</Text>
            </View>
          ...
          <View style={[styles.bottomBar, { backgroundColor: colors.overlay }]}>
            <View style={styles.controlRow}>
              ...
              <View style={styles.controlButtons}>
                <TouchableOpacity style={styles.controlBtn} onPress={() => handleFontSize(-1)}>
            ...
            <TouchableOpacity
              ...
              onPress={() => { setShowControls(false); setShowBookmarks(true); }}
            >
              <Text style={styles.controlLabel}>{'\u2606'} Bookmarks ({bookmarks.length})</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity
                    ...
                    onPress={() => { goToPage(item.page); setShowBookmarks(false); }}
                  >
...
const styles = StyleSheet.create({
  ...
  loadingText: { ...fonts.serifRegular, fontSize: 16, color: colors.parchment, marginTop: 16 },
  loadingSubtext: { ...fonts.sansLight, fontSize: 13, color: colors.textSecondary, marginTop: 4 },
  errorIcon: { fontSize: 36, color: colors.error, marginBottom: 12 },
  errorText: { ...fonts.sansRegular, fontSize: 15, color: colors.textPrimary, textAlign: 'center' },
  retryButton: { marginTop: 20, paddingHorizontal: 24, paddingVertical: 10, borderRadius: borderRadius.md, borderWidth: 1, borderColor: colors.accent },
  retryText: { ...fonts.sansBold, fontSize: 14, color: colors.accent },
  pageArea: { flex: 1 },
  scrollView: { flex: 1 },
  pageContent: { paddingHorizontal: spacing.xxl, paddingTop: spacing.xxxl + 30, paddingBottom: 70 },
  pageText: { fontFamily: 'Georgia' },
  pageIndicator: {
    position: 'absolute', bottom: 0, left: 0, right: 0,
    paddingBottom: 34, paddingTop: 8, paddingHorizontal: spacing.xl, alignItems: 'center',
  },
  pageNumber: { ...fonts.sansLight, fontSize: 12, marginBottom: 4 },
  progressTrack: { width: '100%', height: 2, backgroundColor: 'rgba(128,128,128,0.2)', borderRadius: 1 },
  progressFill: { height: 2, borderRadius: 1 },
  controlsOverlay: { ...StyleSheet.absoluteFillObject, justifyContent: 'space-between' },
  topBar: { flexDirection: 'row', alignItems: 'center', paddingTop: 58, paddingBottom: 12, paddingHorizontal: spacing.lg },
  backButton: { padding: 8 },
  backText: { ...fonts.sansBold, fontSize: 15, color: colors.accent },
  topTitle: { flex: 1, marginHorizontal: 12 },
  topTitleText: { ...fonts.serifBold, fontSize: 15, color: colors.parchment, textAlign: 'center' },
  bookmarkButton: { padding: 8 },
  bookmarkIcon: { fontSize: 24, color: colors.accent },
  bottomBar: { paddingBottom: 40, paddingTop: 16, paddingHorizontal: spacing.xl },
  controlRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 10 },
  controlLabel: { ...fonts.sansRegular, fontSize: 14, color: colors.textPrimary },
  controlButtons: { flexDirection: 'row', alignItems: 'center' },
  controlBtn: {
    width: 40, height: 36, backgroundColor: colors.surface,
    borderRadius: borderRadius.sm, borderWidth: 1, borderColor: colors.surfaceBorder,
    alignItems: 'center', justifyContent: 'center',
  },
  controlBtnText: { ...fonts.sansBold, fontSize: 14, color: colors.accent },
  controlValue: { ...fonts.sansBold, fontSize: 16, color: colors.textPrimary, width: 40, textAlign: 'center' },
  themeBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: borderRadius.sm, borderWidth: 1, borderColor: colors.surfaceBorder, marginLeft: 8 },
  themeBtnActive: { borderColor: colors.accent, borderWidth: 2 },
  themeBtnText: { ...fonts.sansBold, fontSize: 12 },
  bookmarksRow: { paddingVertical: 12, borderTopWidth: 1, borderTopColor: colors.divider, marginTop: 4 },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.7)', justifyContent: 'flex-end' },
  modalContent: { backgroundColor: colors.surface, borderTopLeftRadius: borderRadius.xl, borderTopRightRadius: borderRadius.xl, maxHeight: SCREEN_HEIGHT * 0.6, paddingBottom: 34 },
  ...
  modalTitle: { ...fonts.serifBold, fontSize: 20, color: colors.parchment },
  modalClose: { fontSize: 18, color: colors.textSecondary, padding: 4 },
  emptyBookmarks: { padding: 40, alignItems: 'center' },
  emptyBookmarksText: { ...fonts.sansRegular, fontSize: 15, color: colors.textSecondary },
  bookmarkItem: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: spacing.xl, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: colors.divider },
  bookmarkInfo: { flex: 1 },
  bookmarkLabel: { ...fonts.sansRegular, fontSize: 15, color: colors.textPrimary },
  bookmarkDate: { ...fonts.sansLight, fontSize: 12, color: colors.textMuted, marginTop: 2 },
  bookmarkDelete: { fontSize: 14, color: colors.textMuted, padding: 8 },
});
